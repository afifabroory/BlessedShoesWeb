...
async function generateID(length = 3) {
    ...
    while (true) {
        ...
        if (!await itExists(result).then(isExist => {
...
function nextDataInsert() {
    ...
    var currentIndex = parseInt(document.querySelector("#shoesNo>td>#shoesNo").value)-1;
    ...
    if ((dataLength-1) === (currentIndex+1)) {
        ...
        document.querySelector("#insertUpdate > div > button:last-child").disabled = true;
        document.querySelector("#insertUpdate > div > button:nth-child(1)").disabled = false;
    }
    showData(data, currentIndex+1);
}

function nextDataUpdate() {
    var data = getData("DATA")[document.querySelector("#shoesID>td>#shoesID").value];// Unefficient
    var currentIndex = parseInt(document.querySelector("#shoesNo>td>#shoesNo").value)-1;
    console.log("DATA");
...
function backDataInsert() {
    ...
    if ((currentIndex-1) === 0) {
        ...
        document.querySelector("#insertUpdate > div > button:nth-child(1)").disabled = true;
        document.querySelector("#insertUpdate > div > button:last-child").disabled = false;
    }
}
...
function getInputData() {
     ...
     */
    var data = {};
    data["Service"] = document.querySelector("#shoesService>td>#shoesService").value;
    data["ShoesBrand"] = document.querySelector("#shoesBrand>td>#shoesBrand").value;
...
function cancelOnClickInsert() {
    ...
    document.querySelector(".popup>.popup-buttons>#cancel").removeEventListener("click", cancelOnClickInsert) // Self remove
}

function cancelOnClickUpdate() {
    // Cancel data
    document.querySelector(".popup-container").style.display = "none";
    document.querySelector("#insertupdate").removeEventListener("click", updateOnClick);
    document.querySelector(".popup>.popup-buttons>#cancel").removeEventListener("click", cancelOnClickUpdate) // Self remove
}
...
function updateOnClick() {
    ...
    ///
    var id = document.querySelector("#shoesID>td>#shoesID").value;
    var index = parseInt(document.querySelector("#shoesNo>td>#shoesNo").value)-1;
    var dataLocal = getData("DATA")[id];
    dataLocal[index] = getInputData();
    if (isDone()) dataLocal["timestampOut"] = ServerValue.TIMESTAMP;
    update(id, dataLocal);
}
...
function inputHandler() {
    ...
    document.querySelector("#insertBtn").addEventListener("click", () => {
        ...
        document.querySelector(".popup>.popup-buttons>#cancel").addEventListener("click", cancelOnClickInsert)
...
async function updateHandler() {
    ...
    document.querySelector("#updateBtn").addEventListener("click", () => {
        ...
        document.querySelector(".popup>.popup-buttons>#cancel").addEventListener("click", cancelOnClickUpdate)

        /* Template part */
        var template = document.querySelector("#insertUpdateTemplate").content.cloneNode(true).children[0];
        template.children[1].children[1].remove(); // Beacuse add data are removed, then next button are shiftted to index 1.
        ...
        template.children[0].children[0].children[0].children[1].children[0].addEventListener("input", async () => {
            var query = document.querySelector("#insertUpdate>#input>tbody>#shoesID>td:last-child>input").value.toUpperCase();
            var queryLength = query.length;
            console.log(query)
            document.querySelector("#insertUpdate>#input>tbody>#shoesID>td:last-child>input").value = query;
            ...
            if (queryLength === 3) {
                ...
                if (itExist) {
                    // Shoes Status
                    console.log(data[query]["timestampOut"]);
                    console.log(data[query]);
                    if (data[query]["timestampOut"] === 0) document.querySelector("#shoesStatus>td>#shoesStatus").options[0].selected = true; // In Progress
                    else document.querySelector("#shoesStatus>td>#shoesStatus").options[1].selected = true; // Done
                    
                    var dataLength = -2;
                    console.log("dataLength ")
                    for (var key in data[query]) dataLength++;
                    console.log(dataLength);
                    if (dataLength >= 2) {
                        // Enabled next button
                        document.querySelector("#insertUpdate > div > button:last-child").disabled = false;
                ...
                if (queryLength > 9) {
                    document.querySelector("#insertUpdate>#input>tbody>#shoesID>td:last-child>input").value = "";
                } else {
                    while (queryLength > 3) {
                        document.querySelector("#insertUpdate>#input>tbody>#shoesID>td:last-child>input").value = insertText.slice(0, -1);
                        newLength = document.querySelector("#insertUpdate>#input>tbody>#shoesID>td:last-child>input").value;
                        queryLength = newLength.length;
                    }
                }
                document.querySelector("#insertUpdate > div > button:nth-child(1)").disabled = true;
                document.querySelector("#insertUpdate > div > button:last-child").disabled = true;
                document.querySelector("#shoesStatus>td>#shoesStatus").value = "In Progress"
                clearInputData();
            }
        ...
        document.querySelector("#insertupdate").addEventListener("click", updateOnClick);
    })
}
...
function showData(data, idx = 0) {
    ...
    document.querySelector("#shoesStatus>td>#shoesStatus").value = data[idx]["Status"] // Shoes Status
    document.querySelector("#shoesBrand>td>#shoesBrand").value = data[idx]["ShoesBrand"] // Shoes Brand
    document.querySelector("#shoesSize>td>#shoesSize").value = data[idx]["Size"] // Shoes Size
    document.querySelector("#shoesService>td>#shoesService").value = data[idx]["Service"] // Shoes Service
}
